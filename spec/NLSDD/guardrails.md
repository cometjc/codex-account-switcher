...
- When work is during or after an NLSDD stage and the prompt includes `review`, coordinator should also inspect the execution insights journal and decide whether any open/adopted insight needs planning, adoption, or closure alongside the normal lane review work.
...
- When execution insights exist, prefer `node NLSDD/scripts/nlsdd-summarize-insights.cjs --execution <id>` before or during review/autopilot work so open and adopted runtime learnings do not stay invisible.
