...
echo "Running Claude live refresh via codex-auth..."
cargo run --quiet --manifest-path "${repo_root}/rust/plot-viewer/Cargo.toml" --bin codex-auth -- --refresh-all
...
Next manual TUI smoke check:
  1. cargo run --manifest-path rust/plot-viewer/Cargo.toml --bin codex-auth
