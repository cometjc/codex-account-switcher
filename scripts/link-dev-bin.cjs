...
const sourcePath = path.join(repoRoot, 'bin', 'codex-auth-dev.cjs');
...
try {
  ...
  if (existing.isSymbolicLink()) {
    ...
    if (resolvedLinkedPath === sourcePath) {
      console.log(`codex-auth-dev is already linked at ${targetPath}`);
      process.exit(0);
    }
  }
...
console.log(`Linked codex-auth-dev -> ${sourcePath}`);
