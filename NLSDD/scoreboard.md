...
| plot-mode | Lane 1 | Node contract + handoff | Viewer launch confidence / shell messaging | correction | `1d29843` | `npm run build`; `node --test tests/plot-handoff.test.js`; `node --test tests/plot-mode-shell.test.js` | none | Tighten plot snapshot builder semantics for real 7d history and 5h band math | Keep this lane active: the next refill is still a narrow Node-side contract item, not more shell polish. |
| plot-mode | Lane 2 | Rust runtime + boundary | Boundary API stabilization | correction | `3b62c5b` | `cargo test --manifest-path rust/plot-viewer/Cargo.toml`; `cargo check --manifest-path rust/plot-viewer/Cargo.toml`; `node --test tests/plot-rust-model-contract.test.js` | none | Tighten typed decoding for nested `usage` payload only if another lane proves the runtime blocker | Park this lane after the current correction/re-review closes unless chart or panels work proves the stronger decode path is necessary. |
| plot-mode | Lane 3 | Rust chart surface | ASCII 7d curve surface | correction | `907cbc7` | `cargo test --manifest-path rust/plot-viewer/Cargo.toml render::chart -- --nocapture`; `cargo check --manifest-path rust/plot-viewer/Cargo.toml` | none | Add 5h band, axis labels, and unavailable-band fallback | Keep this lane active after correction closes; the next refill is a clean chart-local rendering item. |
| plot-mode | Lane 4 | Rust panels + docs | Panel docs / README regression | correction | `12785d1` | `node --test tests/plot-readme.test.js`; `cargo test render_panels_builds_visible_summary_and_compare_blocks --manifest-path rust/plot-viewer/Cargo.toml` | none | Add panel-specific regression coverage for the visible Summary / Compare structure | Keep this lane active after correction closes; the next refill is panel-local regression hardening, not broader panel redesign. |
